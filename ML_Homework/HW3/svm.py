...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    p1 = torch.sum(x_1**2, axis=1)[:, None]
    p2 = torch.sum(x_2**2, axis=1)
    p3 = -2 * x_1 @ x_2.T
    p = p1 + p2 + p3
    distances = torch.exp(-p/(2 * sigma**2))
...
def hinge_loss(scores, labels):
    ...
    loss = 1 - scores * labels
    return torch.sum(torch.where(loss >= 0, loss, torch.zeros_like(loss)))
